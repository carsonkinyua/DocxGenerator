...
fn main() {
    ...
    let in_src = "src/docx_glue.rs.in";
    let out_src = Path::new(&out_dir).join("docx_glue.rs");
    ...
    let java_folder = Path::new("../app/src/main/java/com/example/docxgenerator/lib");
    if java_folder.exists() {
        fs::remove_dir_all(java_folder).expect("Could not remove java folder");
    }
    fs::create_dir(java_folder).unwrap();
    let swig_gen = flapigen::Generator::new(LanguageConfig::JavaConfig(
        JavaConfig::new(java_folder.into(), "com.example.docxgenerator.lib".into())
